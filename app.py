...
def initial_state():
    stamp = now_iso()
    return [
        {"username": u["username"], "name": u["name"], "status": "busy", "availableSince": None, "updatedAt": stamp}
        for u in USERS
        if u["role"] == "seller"
    ]
...
def update_status(username, status, actor):
    ...
    if not acting_user or not (acting_user["role"] == "supervisor" or actor == username):
