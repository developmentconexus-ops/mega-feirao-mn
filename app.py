...
USERS = [
    ...
    {"username": "jordana", "name": "Jordana", "password": "jordanaMN", "role": "seller"},
    {"username": "thassya", "name": "Thassya", "password": "thassyaMN", "role": "reception"},
    {"username": "sthella", "name": "Sthella", "password": "SthellaMN", "role": "supervisor"},
    {"username": "leandrot", "name": "Leandro T", "password": "leandrotMN", "role": "supervisor"},
    {"username": "viniciust", "name": "Vinicius T", "password": "viniciustMN", "role": "supervisor"},
    {"username": "oscar", "name": "Oscar", "password": "oscarMN", "role": "viewer"},
    {"username": "daniel", "name": "Daniel", "password": "danielMN", "role": "viewer"},
    {"username": "leandro", "name": "Leandro", "password": "leandroMN", "role": "viewer"},
    {"username": "mauro", "name": "Mauro", "password": "mauroMN", "role": "viewer"},
    {"username": "vinicius", "name": "Vinicius", "password": "viniciusMN", "role": "viewer"},
    {"username": "pedro", "name": "Pedro", "password": "pedroMN", "role": "viewer"},
]
...
:root{--wine:#71152d;--green:#198754;--red:#c0392b;--bg:#f6f2f3;--card:#fff;--text:#251b1e;--muted:#74676b}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:var(--bg);color:var(--text)}button,input,select{font:inherit}.hidden{display:none!important}.page{min-height:100vh}.center{display:grid;place-items:center;padding:24px}.card{background:var(--card);border-radius:22px;box-shadow:0 12px 35px #38101c18}.login{width:min(430px,100%);padding:32px}.brand{width:72px;height:72px;border-radius:20px;background:var(--wine);color:#fff;display:grid;place-items:center;font-weight:900;font-size:26px;margin:auto}.eyebrow{text-transform:uppercase;letter-spacing:.12em;font-size:12px;color:var(--wine);font-weight:800}.login h1{text-align:center;margin:14px 0 4px;color:var(--wine)}.subtitle{text-align:center;color:var(--muted);margin:0 0 28px}.field{display:grid;gap:7px;margin:15px 0;font-weight:700}.field select,.field input{width:100%;padding:14px;border:1px solid #d8cbcf;border-radius:12px;background:#fff}.primary,.danger,.success{border:0;border-radius:14px;padding:15px 18px;color:#fff;font-weight:800;cursor:pointer;width:100%}.primary{background:var(--wine)}.danger{background:var(--red)}.success{background:var(--green)}.error{color:var(--red);font-weight:700;text-align:center}.topbar{background:var(--wine);color:#fff;padding:16px 22px;display:flex;justify-content:space-between;align-items:center}.topbar strong{display:block;font-size:20px}.topbar small{display:block;margin-top:4px;opacity:.86}.link{background:transparent;color:#fff;border:1px solid #ffffff66;border-radius:10px;padding:9px 13px;cursor:pointer}.content{max-width:1100px;margin:auto;padding:22px}.seller-card{max-width:560px;margin:30px auto;padding:28px;text-align:center}.status{font-size:30px;font-weight:900;margin:18px 0}.green{color:var(--green)}.red{color:var(--red)}.position-box{background:#f4edef;border-radius:14px;padding:16px;margin:18px 0;color:var(--muted)}.position-box strong{display:block;color:var(--wine);font-size:24px;margin-top:4px}.next{padding:28px;text-align:center;margin-bottom:22px;border-top:7px solid var(--green)}.next h1{font-size:42px;margin:8px 0;color:var(--wine)}.dot{display:inline-block;width:13px;height:13px;border-radius:50%;margin-right:8px}.available{background:var(--green)}.busy{background:var(--red)}.columns{display:grid;grid-template-columns:1fr 1fr;gap:20px}.list{padding:20px}.list-head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #eee;padding-bottom:12px}.count{color:#fff;border-radius:999px;padding:5px 10px;font-weight:800}.row{display:flex;align-items:center;gap:13px;padding:14px 2px;border-bottom:1px solid #eee}.pos{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:#f1e5e8;color:var(--wine);font-weight:900}.row-main{flex:1}.row-main small{display:block;color:var(--muted);margin-top:4px}.row-action{border:0;border-radius:10px;padding:9px 11px;color:#fff;font-size:12px;font-weight:800;cursor:pointer;white-space:nowrap}.row-action.make-busy{background:var(--red)}.row-action.make-available{background:var(--green)}.row-action:disabled{opacity:.55;cursor:not-allowed}.toast{position:fixed;right:18px;bottom:18px;background:#251b1e;color:#fff;padding:12px 16px;border-radius:12px;box-shadow:0 8px 25px #0003;z-index:20}.empty{color:var(--muted);text-align:center;padding:25px}@media(max-width:760px){.columns{grid-template-columns:1fr}.next h1{font-size:32px}.content{padding:14px}.seller-card{margin:18px auto;padding:22px}.row{align-items:flex-start;flex-wrap:wrap}.row-action{margin-left:47px}}
</style></head><body>
...
<script>
let currentUser=null,stateTimer=null,currentState=null,statusChangePending=false;const $=id=>document.getElementById(id);function show(id){["loginPage","sellerPage","receptionPage"].forEach(x=>$(x).classList.toggle("hidden",x!==id))}function notify(message){$("toast").textContent=message;$("toast").classList.remove("hidden");setTimeout(()=>$("toast").classList.add("hidden"),2200)}async function api(path,options={}){const r=await fetch(path,{cache:"no-store",headers:{"Content-Type":"application/json"},...options});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||"Erro no servidor");return d}function waitText(iso){if(!iso)return"";const m=Math.max(0,Math.floor((Date.now()-new Date(iso).getTime())/60000));return m<1?"agora":`há ${m} min`}async function loadUsers(){const u=await api("/api/users");$("username").innerHTML=u.map(x=>`<option value="${x.username}">${x.name}</option>`).join("")}$("loginForm").addEventListener("submit",async e=>{e.preventDefault();$("loginError").classList.add("hidden");try{currentUser=await api("/api/login",{method:"POST",body:JSON.stringify({username:$("username").value,password:$("password").value})});localStorage.setItem("mega-feirao-user",JSON.stringify(currentUser));startUser()}catch(err){$("loginError").textContent=err.message;$("loginError").classList.remove("hidden")}});function startUser(){clearInterval(stateTimer);if(currentUser.role==="seller"){show("sellerPage");$("sellerName").textContent=currentUser.name}else{show("receptionPage");const labels={viewer:"Somente visualização",reception:"Recepção",supervisor:"Supervisão • pode alterar status"};$("viewMode").textContent=" • "+(labels[currentUser.role]||"")}loadState();stateTimer=setInterval(loadState,1000)}async function loadState(){try{currentState=await api("/api/state");currentUser.role==="seller"?renderSeller():renderReception()}catch(e){console.error(e)}}function renderSeller(){const me=currentState.sellers.find(s=>s.username===currentUser.username);if(!me)return;const a=me.status==="available";$("sellerStatus").textContent=a?"● DISPONÍVEL":"● EM ATENDIMENTO";$("sellerStatus").className="status "+(a?"green":"red");const p=currentState.queue.findIndex(s=>s.username===me.username)+1;$("sellerPosition").innerHTML=a?`Sua posição na fila<strong>${p}º</strong>`:"Você está fora da fila enquanto atende.";$("sellerAction").textContent=a?"INICIAR ATENDIMENTO":"FINALIZAR E FICAR DISPONÍVEL";$("sellerAction").className=a?"danger":"success";$("sellerAction").disabled=statusChangePending}async function toggleStatus(){const me=currentState.sellers.find(s=>s.username===currentUser.username);await changeSellerStatus(currentUser.username,me.status==="available"?"busy":"available")}async function changeSellerStatus(username,status){if(statusChangePending)return;statusChangePending=true;renderReception();if(currentUser.role==="seller")renderSeller();try{await api("/api/status",{method:"POST",body:JSON.stringify({username,status,actor:currentUser.username})});await loadState();notify(status==="busy"?"Vendedor marcado em atendimento.":"Vendedor marcado como disponível.")}catch(error){notify(error.message)}finally{statusChangePending=false;if(currentUser.role==="seller")renderSeller();else renderReception()}}function supervisorButton(s){if(currentUser.role!=="supervisor")return"";const makeBusy=s.status==="available";return `<button class="row-action ${makeBusy?"make-busy":"make-available"}" ${statusChangePending?"disabled":""} onclick="changeSellerStatus('${s.username}','${makeBusy?"busy":"available"}')">${makeBusy?"MARCAR ATENDENDO":"MARCAR DISPONÍVEL"}</button>`}function renderReception(){if(!currentState)return;const q=currentState.queue,b=currentState.sellers.filter(s=>s.status==="busy"),n=q[0];$("nextSeller").textContent=n?n.name:"Nenhum disponível";$("nextWait").textContent=n?`Disponível ${waitText(n.availableSince)}`:"Aguardando alguém entrar na fila.";$("queueCount").textContent=q.length;$("busyCount").textContent=b.length;$("queueList").innerHTML=q.length?q.map((s,i)=>`<div class="row"><span class="pos">${i+1}</span><div class="row-main"><strong>${s.name}</strong><small>Disponível ${waitText(s.availableSince)}</small></div><span class="dot available"></span>${supervisorButton(s)}</div>`).join(""):"<p class='empty'>Fila vazia</p>";$("busyList").innerHTML=b.length?b.map(s=>`<div class="row"><div class="row-main"><strong>${s.name}</strong><small>Em atendimento</small></div><span class="dot busy"></span>${supervisorButton(s)}</div>`).join(""):"<p class='empty'>Todos disponíveis</p>"}function logout(){clearInterval(stateTimer);localStorage.removeItem("mega-feirao-user");currentUser=null;$("password").value="";show("loginPage")}(async()=>{await loadUsers();const s=localStorage.getItem("mega-feirao-user");if(s){try{currentUser=JSON.parse(s);startUser()}catch{show("loginPage")}}else show("loginPage")})()
</script></body></html>'''
...
def read_state():
    ...
    for user in USERS:
        ...
        if current:
            ...
            current.setdefault("updatedBy", "system")
            current.setdefault("updatedByRole", "system")
...
def update_status(username, status, actor):
    if status not in {"available", "busy"}:
        raise ValueError("Status inválido")
    target = user_by_username(username)
    acting_user = user_by_username(actor)
    if not target or target["role"] != "seller":
        raise ValueError("Vendedor não encontrado")
    if not acting_user:
        raise ValueError("Usuário não identificado")
    if acting_user["role"] == "supervisor":
        pass
    elif acting_user["role"] == "seller" and actor == username:
        pass
    else:
        raise ValueError("Usuário sem permissão para alterar este vendedor")
    with LOCK:
        state = read_state()
        stamp = now_iso()
        for seller in state:
            ...
            seller["updatedBy"] = actor
            seller["updatedByRole"] = acting_user["role"]
            break
        write_state(state)
        return state
